            ...
            <%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
                <%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
                    ...
                    <body class="bg-white">
                        ...
                        <!-- Hidden form with order data for EmailJS -->
                        <div style="display: none;">
                            <input type="hidden" id="customerEmail" value="<c:out value='${customerEmail}' />">
                            <input type="hidden" id="customerName" value="<c:out value='${customerName}' />">
                            <input type="hidden" id="orderId" value="<c:out value='${orderId}' />">
                            <input type="hidden" id="orderDate" value="<c:out value='${orderDate}' />">
                            <input type="hidden" id="shippingAddress" value="<c:out value='${shippingAddress}' />">
                            <input type="hidden" id="subtotal" value="<fmt:formatNumber value='${subtotal}' pattern='#,##0.00'/>">
                            <input type="hidden" id="discount" value="<fmt:formatNumber value='${discount}' pattern='#,##0.00'/>">
                            <input type="hidden" id="discountPercentage" value="<c:out value='${discountPercentage}' />">
                            <input type="hidden" id="shipping" value="<fmt:formatNumber value='${shipping}' pattern='#,##0.00'/>">
                            <input type="hidden" id="total" value="<fmt:formatNumber value='${total}' pattern='#,##0.00'/>">
                            <input type="hidden" id="trackingUrl" value="${ctx}/management/order/details/${orderId}">
                            
                            <!-- Order items data -->
                            <div id="orderItemsData">
                                <c:forEach items="${orderItems}" var="item" varStatus="status">
                                    <c:if test="${status.index < fn:length(orderItems) - 1}">
                                        <div class="order-item-data"
                                             ...
                                             data-quantity="<c:out value='${item.quantity}' />"
                                             data-subtotal="<c:out value='${item.subtotal}' />"
                                             data-style="<c:out value='${item.style}' />"
                                             data-size="<c:out value='${item.size}' />"
                                             data-color="<c:out value='${item.color}' />"
                                             data-image-url="<c:out value='${item.imageUrl}' />">
                                        </div>
                                    </c:if>
                                </c:forEach>
                            </div>
                        ...
                        <script type="text/javascript">
                            ...
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                // Check if this order's email has already been sent
                                const sentOrderEmails = sessionStorage.getItem('sentOrderEmails') ? 
                                    JSON.parse(sessionStorage.getItem('sentOrderEmails')) : [];
                                
                                // Only send email if it hasn't been sent for this order
                                if (!sentOrderEmails.includes(orderId)) {
                                    // Send order confirmation email
                                    sendOrderConfirmationEmail(orderId);
                                    
                                    // Add this order to the sent list
                                    sentOrderEmails.push(orderId);
                                    sessionStorage.setItem('sentOrderEmails', JSON.stringify(sentOrderEmails));
                                } else {
                                    console.log('Email for order #' + orderId + ' was already sent. Skipping.');
                                }
                            });
                            ...
                            function sendOrderConfirmationEmail(orderId) {
                                ...
                                // Get order items from hidden divs
                                const orderItems = [];
                                const orderItemElements = document.querySelectorAll('.order-item-data');
                                
                                orderItemElements.forEach(function(item) {
                                    orderItems.push({
                                ...
                                emailjs.send("uongngocson", "template_qlqdea9", emailParams)
                                    .then(function(response) {
                                        ...
                                        showNotification("Đã gửi email xác nhận đơn hàng đến " + customerEmail, true);
                                    })
                                    .catch(function(error) {
                                        console.error("Failed to send order confirmation email", error);
                                        showNotification("Không thể gửi email xác nhận. Vui lòng thử lại sau.", false);
                                    });
                            }
                            ...
                            function showNotification(message, isSuccess) {
                                ...
                                notification.className = isSuccess 
                                    ? 'email-notification success-notification' 
                                    : 'email-notification error-notification';
