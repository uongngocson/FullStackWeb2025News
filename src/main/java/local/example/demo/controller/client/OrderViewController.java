...
// @RequestParam(value = "variantIds", required = false) List<Integer>
// variantIds,
// @RequestParam(value = "quantities", required = false) List<Integer>
// quantities,
...
// if (variantIds != null && quantities != null && variantIds.size() ==
// quantities.size()) {
...
// customerService.findCustomerById(currentCustomer.getCustomerId());
// model.addAttribute("customer", customerDetails);

// // Get customer addresses and pass them directly to the view
// List<Address> addresses =
// addressService.getAddressesForCustomer(customerDetails.getCustomerId());
...
// log.info("Retrieved {} addresses for customer ID: {}", addresses.size(),
...
// address.getWardId(),
// (address.getGhnWard() != null) ? address.getGhnWard().getWardName() :
// address.getWard());

// log.info("District: ID={}, Name={}",
// address.getDistrictId(),
// (address.getGhnDistrict() != null) ?
// address.getGhnDistrict().getDistrictName() : address.getDistrict());

// log.info("Province: ID={}, Name={}",
// address.getProvinceId(),
// (address.getGhnProvince() != null) ?
// address.getGhnProvince().getProvinceName() : address.getProvince());
// }
// } else {
...
// public String processCheckout(@RequestParam("selectedItems") List<Integer>
...
// // Get selected cart details and convert to the format needed for the order
// page
...
// log.info("Address formatted: ID={}, Recipient={}, Phone={}, Street={},
...
// // // Calculate discount percentage if both subtotal and discount are
// provided
...
// // // If we have an order ID but no items, try to fetch them from the
// database
...
// // item.setSubtotal(detail.getPrice().multiply(new
// BigDecimal(detail.getQuantity())));
...
// // System.err.println("Error processing order confirmation: " +
// e.getMessage());
// // e.printStackTrace();
...
// * Used when redirected from payment gateway or for direct access with order
// ID
// */
...
// // if (orderId == null || orderId.isEmpty()) {
// // model.addAttribute("error", "Không tìm thấy thông tin đơn hàng. Vui lòng
// kiểm tra lại.");
// // return "client/user/orderconfirm";
// // }
...
// 'at' hh:mm a"))
// // : LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd MMM yyyy 'at'
// hh:mm a"));

// // // Get shipping address if available
// // String shippingAddress = "Không có thông tin địa chỉ";
// // // You would need to get the shipping address from your order or address
// model
...
// // model.addAttribute("paymentMessage", "Thanh toán thất bại. Vui lòng liên
// hệ hỗ trợ khách hàng.");
...
// e.getMessage());
// // e.printStackTrace();
// // model.addAttribute("error", "Đã xảy ra lỗi khi tải thông tin đơn hàng. Vui
// lòng thử lại sau.");
// // return "client/user/orderconfirm";
// // }
// // }
